...
class lib(object):
    def __init__(self):
        ...
        self.lock = threading.RLock()  #ACRESCENTADO#
...
class JanelaGrafica(QtGui.QMainWindow):
    ...
    def sinais(self):
        ...
        ## Nova aba 4.2 - Rampa Corrente Multipla ##
        self.ui.pB_main_coil.clicked.connect(self.main_current)
        self.ui.pB_trim_coil.clicked.connect(self.trim_current)
        self.ui.pB_ch_coil.clicked.connect(self.ch_current)
        self.ui.pB_cv_coil.clicked.connect(self.cv_current)
        self.ui.pB_refresh_curr.clicked.connect(self.Display_Fonte_Mult)
##        self.ui.pB_qs_coil.clicked.connect(self.qs_current)
...
##        self.xyx = eixos()
    ...
    def Carregar_Fonte(self):
        ...
        try:
            ...
            self.ui.tabWidget_2.setTabEnabled(1,False)          # ACRESCENTADO - Fontes multiplas apenas para digital
...
##            print(reshape)
    ...
    def Habilitar_trim(self):                               #ACRESCENTADO#
        if lib.controle_fonte == 1:
            self.ui.tabWidget_2.setTabEnabled(1,True)       #Libera aba multiplas fontes
            if self.ui.check_aux.isChecked():
                ...
                self.ui.status_fonte_mult.setEnabled(True)
                self.ui.label_194.setEnabled(True)
                self.Corrente_Fonte_Mult()
##                self.main_current()
##                Monitorar = Monitor_Thd_Fontes()            #Enable Threading
                ...
                self.ui.status_fonte_mult.setEnabled(False)
                self.ui.label_194.setEnabled(False)
        else:
...
##        atual = round(lib.PUC.ReadDA(),3)
    ...
    def Hab_Auto(self):
        ...
        ########### ABA 4.2: Corrente Multipla  ###########
    def Verificar_Limite_Corrente_Mult(self):
        pass

    def Corrente_Fonte_Mult(self):
        try:
##            monitorar = Monitor_Thd_Fontes()
            self.endereco = []
            self.main_current()
               
##            if self.ui.pB_ch_coil.isChecked() == False and self.ui.pB_cv_coil.isChecked() == False and self.ui.pB_qs_coil.isChecked() == False and self.ui.pB_trim_coil.isChecked() == False:
##                self.addr_main = self.ui.Addr_main.value()
##                lib.Digital.SetSlaveAdd(self.addr_main)
##                self.ui.status_fonte_mult.setText('Main:Ligada   End. Atual: '+str(self.addr_main))
##                return True
##            else:
##                return False
        except:
...
##        monitorar = Monitor_Thd_Fontes()
        self.ui.pB_main_coil.setChecked(True)
        self.addr_main = self.ui.Addr_main.value()
        self.endereco.append(self.addr_main)
        lib.Digital.SetSlaveAdd(self.addr_main)        
        if self.ui.pB_main_coil.isChecked():
            lib.Digital.SetSlaveAdd(self.addr_main)
        else:
            self.endereco = []
            self.ui.status_fonte_mult.setText('Main:Desligada   End. Atual: '+str(self.addr_main))

        self.Display_Fonte_Mult()  # Rever isso #

    def trim_current(self):
##        monitorar = Monitor_Thd_Fontes()
        self.ui.check_TrimCoil.setChecked(True)
        self.addr_trim = self.ui.Addr_trim.value()
        self.endereco.append(self.addr_trim)
        lib.Digital.SetSlaveAdd(self.addr_trim)
        if self.ui.pB_trim_coil.isChecked():
            lib.Digital.TurnOn()
            time.sleep(0.5)
            status = lib.Digital.Read_ps_OnOff()
            if status == 1:
                lib.Status_Fonte_ch = 1
                lib.Corrente_Atual_ch = 0
                self.ui.status_fonte_mult.setText('Trim: Ligada   End. Atual: '+str(self.addr_trim))
                self.ui.tipo_fonte.setText('Digital trim')
            else:
                self.ui.pB_ch_coil.setChecked(False)
                QtGui.QMessageBox.critical(self,'Atenção.','Fonte Não Ligou.',QtGui.QMessageBox.Ok)
                status = lib.Digital.Read_ps_OnOff()
                return
        else:
            lib.Digital.TurnOff()
            self.endereco = []
            self.ui.check_chCoil.setChecked(False)
            self.ui.status_fonte_mult.setText('Cv:Desligada   End. Atual: '+str(self.addr_ch))

        self.Display_Fonte_Mult()
   
    def ch_current(self):
##        monitorar = Monitor_Thd_Fontes()
        self.ui.check_chCoil.setChecked(True)
        self.addr_ch = self.ui.Addr_ch.value()
        self.endereco.append(self.addr_ch)
        lib.Digital.SetSlaveAdd(self.addr_ch)
        if self.ui.pB_ch_coil.isChecked():
            lib.Digital.TurnOn()
            time.sleep(0.5)
            status = lib.Digital.Read_ps_OnOff()
            if status == 1:
                lib.Status_Fonte_ch = 1
                lib.Corrente_Atual_ch = 0
                self.ui.status_fonte_mult.setText('Ch: Ligada   End. Atual: '+str(self.addr_ch))
                self.ui.tipo_fonte.setText('Digital Ch')
            else:
                self.ui.pB_ch_coil.setChecked(False)
                QtGui.QMessageBox.critical(self,'Atenção.','Fonte Não Ligou.',QtGui.QMessageBox.Ok)
                status = lib.Digital.Read_ps_OnOff()
    ...
    def cv_current(self):
##        monitorar = Monitor_Thd_Fontes()
        self.ui.check_cvCoil.setChecked(True)
        self.addr_cv = self.ui.Addr_cv.value()
        self.endereco.append(self.addr_cv)
        lib.Digital.SetSlaveAdd(self.addr_cv)
        if self.ui.pB_cv_coil.isChecked():
            lib.Digital.TurnOn()
            time.sleep(0.5)
            status = lib.Digital.Read_ps_OnOff()
            if status == 1:
                lib.Status_Fonte_cv = 1
                lib.Corrente_Atual_cv = 0
                self.ui.status_fonte_mult.setText('Cv: Ligada   End. Atual: '+str(self.addr_cv))
                self.ui.tipo_fonte.setText('Digital Cv')
            else:
                self.ui.pB_cv_coil.setChecked(False)
                QtGui.QMessageBox.critical(self,'Atenção.','Fonte Não Ligou.',QtGui.QMessageBox.Ok)
                status = lib.Digital.Read_ps_OnOff()
                return
        else:
            lib.Digital.TurnOff()
            self.endereco = []
            self.ui.check_cvCoil.setChecked(False)
            self.ui.status_fonte_mult.setText('Cv:Desligada   End. Atual: '+str(self.addr_cv))

        self.Display_Fonte_Mult()

    def Display_Fonte_Mult(self):
        try:
##            self.lock.acquire()
            if self.ui.check_chCoil.isChecked() and not self.ui.check_cvCoil.isChecked():
                lib.Digital.SetSlaveAdd(self.addr_ch)
                corrente = round(float(lib.Digital.Read_iLoad1()), 3)
                self.ui.display_ch.display(corrente)

            elif self.ui.check_cvCoil.isChecked() and not self.ui.check_chCoil.isChecked():
                lib.Digital.SetSlaveAdd(self.addr_cv)
                corrente = round(float(lib.Digital.Read_iLoad1()), 3)
                self.ui.display_cv.display(corrente)

            elif self.ui.check_cvCoil.isChecked() and self.ui.check_chCoil.isChecked():
                for item in self.endereco:
                    lib.Digital.SetSlaveAdd(int(item))
                    corrente = round(float(lib.Digital.Read_iLoad1()), 3)
                    if item == 2:
                        self.ui.display_ch.display(corrente)
                    else:
            ...
            traceback.print_exc(file=sys.stdout)
##            self.lock.release()
            QtGui.QMessageBox.critical(self,'Erro','Erro na leitura das correntes.',QtGui.QMessageBox.Ok)
            return False
...
#################### Leitura Corrente Secundária Fonte Digital 10A (Multicanal) ############################# ACRESCENTADO
...
###################### Leitura Corrente CH ############################# ACRESCENTADO
##class leitura_corrente_ch(threading.Thread):
##    def __init__(self, nVoltas, velocidade):
##        threading.Thread.__init__(self)
##        self.nVoltas = int(nVoltas)
##        self.velocidade = float(velocidade)
...
##        self.saida = saida
##
###################### Leitura Corrente CV ############################# ACRESCENTADO
##class leitura_corrente_cv(threading.Thread):
##    def __init__(self, nVoltas, velocidade):
##        threading.Thread.__init__(self)
##        self.nVoltas = int(nVoltas)
##        self.velocidade = float(velocidade)
...
#################### Monitorando CV, CH, QS ############################# ACRESCENTADO
class Monitor_Thd_Fontes(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.setDaemon(True)
...
##                time.sleep(0.5)
                print('ok2')
                if lib.Janela.ui.check_chCoil.isChecked() == True and lib.Janela.ui.check_cvCoil.isChecked() == False:
                    time.sleep(0.5)
                    lib.Digital.SetSlaveAdd(self.addr_ch)
                    corrente = round(float(lib.Digital.Read_iLoad1()), 3)
                    lib.Janela.ui.display_ch.display(corrente)
                    print('ok3')
                else:
                    print('else1')
                
                if lib.Janela.ui.check_cvCoil.isChecked() == True and lib.Janela.ui.check_chCoil.isChecked() == False:
                    time.sleep(0.5)
                    lib.Digital.SetSlaveAdd(self.addr_cv)
                    corrente = round(float(lib.Digital.Read_iLoad1()), 3)
                    lib.Janela.ui.display_cv.display(corrente)
                    print('ok4')
                else:
                    print('else2')
                
                if lib.Janela.ui.check_cvCoil.isChecked() == True and lib.Janela.ui.check_chCoil.isChecked() == True:
                    time.sleep(0.5)
                    print('ok5')
                    for item in self.endereco:
                        lib.Digital.SetSlaveAdd(int(item))
                ...
                if lib.Janela.ui.check_TrimCoil.isChecked() == True and lib.Janela.ui.check_chCoil.isChecked() == False and lib.Janela.ui.check_cvCoil.isChecked() == False:
##                    time.sleep(0.5)
                    lib.Digital.SetSlaveAdd(self.addr_trim)
                    corrente = round(float(lib.Janela.Converte_DCCT_MultiCanal()), 3)
                    lib.Janela.ui.display_trim.display(corrente)
                else:
                    print('else_trim')
    
                if lib.Janela.ui.check_chCoil.isChecked() == False and lib.Janela.ui.check_cvCoil.isChecked() == False and lib.Janela.ui.check_qsCoil.isChecked() == False and lib.Janela.ui.check_TrimCoil.isChecked() == False:
                    print('ok6')
                    lib.Digital.SetSlaveAdd(int(3))
                    print('SlaveAdd = 3 ')
##                    time.sleep(0.5)
##                    lib.Janela.ui.status_fonte_mult.setText('Main:Ligada   End. Atual: 3')
                    self.Fechar = False
                    print(self.Fechar)
                else:
                    print('else4')
            ...
            self.Fechar = False
            self.lock.release()
            QtGui.QMessageBox.critical(lib.Janela,'Erro','Erro na leitura das correntes.',QtGui.QMessageBox.Ok)
            return False
...
##            if (lib.Digital_Conectada == 1) and (lib.Status_Fonte == 1) and (lib.Fonte_Pronta == 1):                
##                status = lib.Digital.Read_ps_SoftInterlocks()
##                time.sleep(.5)
##                status2 = lib.Digital.Read_ps_HardInterlocks()
##                time.sleep(.5)
