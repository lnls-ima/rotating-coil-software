...
import SerialDRS_2          ## Biblioteca Fonte Digital 1000A
...
class JanelaGrafica(QtGui.QMainWindow):
    ...
    def sinais(self):
        ...
        self.ui.label_173.setVisible(False)                     #ACRESCENTADO#
        self.ui.label_174.setVisible(False)                     #ACRESCENTADO#      
        self.load_default()
            ...
            try:
                if lib.controle_fonte == 0:                     # Se a combobox estiver selecionada para a PUC, ou seja, lib.controle_fonte = 0   #ACRESCENTADO#
                    ...
                    try:
                        ...
                        lib.Digital = SerialDRS_2.SerialDRS_FBP()
                        ...
                        self.ui.Defasagem_Senoidal.setEnabled(True)
                        self.ui.Posicao_Final_Senoidal.setEnabled(True)
                        for i in range(3,7): ## Abas curvas nao estão prontas
                            self.ui.tabWidget_3.setTabEnabled(i,False)
...
##                    lib.Digital.TurnOn()                    # Liga saída da fonte
                    ...
                    Fonte_2 = SerialDRS_2.Std_Alone(lib.Digital)    ## ALTERADO FONTE 1000A
                    Fonte_2.On_Off()                                ## ALTERADO FONTE 1000A
...
##            print(reshape)
    ...
    ##                QtGui.QApplication.processEvents()
                ...
                if self.ui.cb_DCCT.isChecked():
                    ...
                    self.ui.label_161.setEnabled(True)
                    self.ui.label_164.setEnabled(True)
                    corrente4 = round(self.ConverteDCCT2(), 3)
    ...
    def ConverteDCCT(self):                     #ACRESCENTADO# Para cabeça de 160A
        self.variavel = lib.GPIB.Coleta_Multimetro()
        self.num = (float(self.variavel)*10)/0.625
        return self.num

    def ConverteDCCT2(self):                     #ACRESCENTADO# Para cabeça de 40A
        self.variavel = lib.GPIB.Coleta_Multimetro()
...
class leitura_corrente_multimetro(threading.Thread):
    def __init__(self, nVoltas, velocidade):
        ...
        self.start()
    def run(self):
        if lib.controle_fonte == 0:              #ACRESCENTADO# Puc e DCCT 160A
            saida = []
            for i in range(self.nVoltas):
                saida.append(lib.Janela.ConverteDCCT())
                time.sleep((1/self.velocidade)-0.01)
            self.saida = saida
        else:                                   #ACRESCENTADO# Digital e DCCT 40A
            saida = []
            for i in range(self.nVoltas):
                saida.append(lib.Janela.ConverteDCCT2())
                time.sleep((1/self.velocidade)-0.01)
            self.saida = saida
