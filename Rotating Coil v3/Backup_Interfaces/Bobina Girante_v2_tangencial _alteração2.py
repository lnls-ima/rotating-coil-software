...
##            QtGui.QMessageBox.warning(lib.Janela,'Aviso.','Arquivo de inicialização inexistente.',QtGui.QMessageBox.Ok)
        ...
        self.ui.Porta.setCurrentIndex(int(config[1][1][1])-1)
        self.ui.Porta_2.setCurrentIndex(int(config[1][2][1])-1)
        self.ui.Porta_3.setCurrentIndex(int(config[1][3][1])-1)
        self.ui.Tipo_PUC.setCurrentIndex(int(config[1][4][1]))
        self.ui.Porta_4.setCurrentIndex(int(config[1][5][1])-1)
        self.ui.Enderac_PUC.setValue(int(config[1][6][1]))
    ...
    def CONECTAR(self):        
        if (self.ui.conectar.text() == 'Conectar'):
            ...
            try:
                ...
                QtGui.QMessageBox.critical(self,'Erro.','Erro ao Desconectar a PUC. \nVerificar Equipamento.',QtGui.QMessageBox.Ok)
...
##                corrente2 = round(self.Valor_Equacao(1,self.ConverteDCCT(),float(self.ui.Fator_Entrada.text())),3)
    ...
    def ConverteDCCT(self):                     #ACRESCENTADO#
        ...
        self.num = (float(self.variavel)*10)/0.625
        return self.num
...
##                Amp = self.Verificar_Limite_Corrente(2,abs(float(self.ui.Amplitude_Curva_Amortecida.text())),self.offset)
##                if (Amp == 'False'):
##                    self.ui.Amplitude_Curva_Amortecida.setText('0')
##                    return False
##                self.ui.Amplitude_Curva_Amortecida.setText(str(Amp))    
        except:
            QtGui.QMessageBox.warning(self,'Atenção.','Verifique Valor do Parâmetro da Amplitude da Curva.',QtGui.QMessageBox.Ok)
            return False
        ...
        if Tipo_Curva == 0 or Tipo_Curva == 1: #or Tipo_Curva == 2:
...
##        if Tipo_Curva == 6:
##            try:
##                endereco_arquivo_curva_arbitraria = 'teste_real.dat'
####                endereco_arquivo_curva_arbitraria = 'teste_senoide.dat'
####                endereco_arquivo_curva_arbitraria = 'teste_senoide_1.dat'
##                dados = numpy.loadtxt(endereco_arquivo_curva_arbitraria)
##                x = []
##                x.append(dados)
##                dados = []
##                dados = x[0]
##                if (len(dados)*total_ciclo)>(const.Pontos_Puc):
##                    QtGui.QMessageBox.warning(self,'Atenção.','Quantidade de Pontos da Curva Arbitrária Execida. Verificar Dados ou Número de Ciclos.',QtGui.QMessageBox.Ok)
##                    return False
##                for i in range(len(dados)):
##                    if dados[i]>1:  #Verifica dados unitarios
##                        QtGui.QMessageBox.warning(self,'Atenção.','Dados do Arquido da Curva Arbitrária Não são de Valor Unitário. Verificar Dados.',QtGui.QMessageBox.Ok)
##                        return False
##            except:
##                QtGui.QMessageBox.warning(self,'Atenção.','Dados do Arquido da Curva Arbitrária Inválidos.',QtGui.QMessageBox.Ok)
##                return False
        ...
        if (lib.Modelo_PUC == 0):
            ...
            pontos = list(map(lambda x: Amp*numpy.sin((w*x))+self.offset,range(const.Pontos_Puc)))  # gera a lista de pontos (EQUACAO)
...
##                    print('Frequencia PUC:', freq_puc)
##                    print('Divisor:', lib.Divisor_Puc)
##                    print('Frequencia do sinal:', freq)
##                    print('total_ciclo:', total_ciclo)
##                    print('ciclos_total:', ciclos_total)
##                    print('lib.Ciclos_Puc:', lib.Ciclos_Puc)
##                    print(len(lista2))
##                    print(lista2[:10])
##                    print(len(pontos))
##                    print(len(temp_ms))
...
##    Endereco(1-Giro;2-Mesa longitudinal) sentido(0-Horario;1-Antihorario) modo(0-manual;1-continuo)
    ...
    def CARREGABOBINA(self):
        try:
            arquivo = QtGui.QFileDialog.getOpenFileName(self, 'Carrergar Arquivo Bobina', '.','Data files (*.dat);;Text files (*.txt)')
            f = open(arquivo, 'r')
        except:
            return
        leitura = f.read().strip()
        ...
        try:
            ...
            self.ui.observacao_bobina.setPlainText(str(dados[10]))
...
##            arquivo = QtGui.QFileDialog.getSaveFileName(self, 'Save File', nome,'Data files (*.dat);;Text files (*.txt)')
            arquivo = str(arquivo) + '_' + str(bob_excit) + '_BOB_' + str(Corrente_Principal).zfill(5) + 'A_' + str(data_nome) + '_' + str(hora_nome) + '.dat'
...
##            f.write('corrente_alim_secundaria_avg(A)\t' + str('{0:+0.6e}'.format(numpy.mean(lib.LeituraCorrente.saida))) + '\n')
##            f.write('corrente_alim_secundaria_std(A)\t' + str('{0:+0.6e}'.format(numpy.std(lib.LeituraCorrente.saida))) + '\n')
...
##        lib.LeituraCorrente_Secundaria = leitura_corrente_multimetro(lib.voltas_offset,lib.velocidade)
    else:
...
##### Calculo da Normalização dos Multipolos @ 0.0175 para Tipo de Ima #####
    lib.Nnl = numpy.zeros(21)
    lib.Snl = numpy.zeros(21)
    r_ref = 0.0175
    for i in range(1,nmax,1):
