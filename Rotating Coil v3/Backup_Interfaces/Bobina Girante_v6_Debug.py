...
##        self.ui.tabela.clear()
        TipoIma = self.ui.TipoIma.currentIndex()
        if len(lib.F[0])>16:
            nmax = 16
        else:
            nmax = 8
    ...
    def deslocamento(self):
        ordens = numpy.array([])
        posicao = int(self.ui.TipoIma.currentIndex())
...
##                    self.ui.tabWidget.setCurrentIndex(9)
                    self.ui.tabela.setEnabled(True)
                    self.ui.salvar_harmonico.setEnabled(True)
                    self.ui.Emergencia_9.setEnabled(True)
                    self.Tabela()
...
##                    pass
        ...
        if tipo_coleta == 1: #Rotina de Coleta de corrente manual com coletas Sucessivas
            ...
            if (self.ui.tabWidget.isTabEnabled(3) == True):
...
##                bob_excit = 'K'
##
##            imabobina = self.ui.ima_bobina.currentIndex()     #ACRESCENTADO#
...
##            if self.ui.check_aux.isChecked():             #ACRESCENTADO#
...
##            print(arquivo)
            ...
            if imabobina == 1 and self.ui.check_aux.isChecked() and TipoIma == 3:             #PARA SEXTYPOLO DO ANEL S15#
                if self.ui.check_chCoil.isChecked():              #CH Coil
                ...
                if self.ui.check_cvCoil.isChecked():              #CV Coil
                ...
                if self.ui.check_qsCoil.isChecked():              #QS Coil
                    f.write('ch_coil_current_avg(A)       \t' + str('{0:+0.6e}'.format(numpy.mean(Corrente_2))) + '\n')
                    f.write('ch_coil_current_std(A)       \t' + str('{0:+0.6e}'.format(numpy.std(Corrente_2))) + '\n')
                    f.write('cv_coil_current_avg(A)       \t' + str('{0:+0.6e}'.format(numpy.mean(Corrente_2))) + '\n')
                    f.write('cv_coil_current_std(A)       \t' + str('{0:+0.6e}'.format(numpy.std(Corrente_2))) + '\n')
            ...
            elif imabobina == 1 and self.ui.check_aux.isChecked() and TipoIma == 2:             #PARA QUADRUPOLO DO ANEL Q14, Q20, Q30#
...
def ColetaDados_Leitura(Repeticao):
    if self.ui.Hab_Corretora.isChecked() or self.ui.cb_DCCT.isChecked():                
        lib.LeituraCorrente = leitura_corrente_multimetro(lib.voltas_offset,lib.velocidade)
        if lib.controle_fonte == 1 and self.ui.check_aux.isChecked():
            lib.LeituraCorrente_Secundaria = leitura_corrente_sec_mult(lib.voltas_offset,lib.velocidade)        #ACRESCENTADO#

        if lib.controle_fonte == 1 and self.ui.check_aux.isChecked() and self.ui.check_TrimCoil.isChecked():
...
##    sentido = self.ui.sentido_2.currentIndex()
##    if sentido == 0:
##        lib.sentido = 'Horario'
##    else:
...
def ColetaDados_Calculos(Repeticao):
    ...
    lib.Bucked = self.ui.Bucked.isChecked()
    Ne = int(self.ui.nespiras.text())
    Neb = int(self.ui.nespirasb.text())
    r1 = float(self.ui.raio1.text())
    r2 = float(self.ui.raio2.text())
    r1b = float(self.ui.raio1b.text())
    r2b = float(self.ui.raio2b.text())
    TipoIma = self.ui.TipoIma.currentIndex()
    volta_filtro = int(self.ui.filtro_voltas.text())
    aux = []
...
############ Filtro Voltas Iniciais e Finais ############
    for i in range(int(self.ui.descarte_inicial.text())):
        aux.pop(0)  ### despreza voltas de aceleração
    for i in range(int(self.ui.descarte_final.text())):
...
##### Plota os graficos na aba de resultados e habilita botoes de coleta #####
        if Repeticao == 1:
            ...
            self.ui.tabWidget.setTabEnabled(const.numero_de_abas-1,True)
            self.ui.groupBox_2.setEnabled(True)
            self.ui.coletar.setEnabled(True)
            if lib.Ref_Bobina == 0:
...
def Historico_Falhas(Index):
    ...
    hora = time.strftime("%H:%M:%S", time.localtime())
    Nome = str(self.ui.Nome_Ima.text())
    Medida = 1+int(str(self.ui.label_138.text()))
...
class EncontraRef_ND760(threading.Thread):
    ...
    def callback(self):
        self._stop()
    def run(self):
        self.ui.status.setText('Movendo')
        self.ui.groupBox_6.setEnabled(False)
...
class motortransversal(threading.Thread):
    ...
    def run(self):
        ...
        if self.ui.bobinadesmontada.isChecked():
            ...
            self.ui.groupBox_4.setEnabled(True)
        self.ui.zerar.setEnabled(True)
        self.ui.bobinadesmontada.setEnabled(True)
        self.ui.status.setText('Pronto')
        QtGui.QApplication.processEvents()
        lib.Motor_Posicao = 1
#################################################################################################     
...
class leitura_corrente_puc(threading.Thread):
    ...
    ##        if (self.ui.tabWidget.isTabEnabled(3) == True):
            if (lib.PUC_Conectada == 1) and (lib.Status_Fonte == 1):
                fator = float(self.ui.Fator_Entrada.text())
...
class Controle_Status(threading.Thread):
    ...
    def run(self):
        ...
        while i==0:
            ...
            ### Status Fonte ###
            if (lib.Status_Fonte == 1):
                tela.myapp.ui.status_fonte.setText('Ligada')
                tela.myapp.ui.ligar_fonte.setText('Desligar')
            else:
                tela.myapp.ui.status_fonte.setText('Desligada')
                tela.myapp.ui.ligar_fonte.setText('Ligar')
            if (lib.controle_fonte == 0):                   #ACRESCENTADO# Selecionda a PUC    
                if (lib.PUC_Conectada == 1) and (lib.Status_Fonte == 1) and (lib.Fonte_Pronta == 1) and (lib.Fonte_Calibrada == [1,1]):
                    tela.myapp.ui.label_135.setText('OK')
                else:
                    tela.myapp.ui.label_135.setText('NOK')
            else:                                           #ACRESCENTADO# Seleciona a Digital
            ...
            if (lib.Fonte_Calibrada[1] == 1):
                tela.myapp.ui.label_equacao_saida.setText('Saída: OK')
            else:
                tela.myapp.ui.label_equacao_saida.setText('Saída: NOK')
                
            if (lib.Fonte_Calibrada[0] == 1):
                tela.myapp.ui.label_equacao_entrada.setText('Entrada: OK')
            else:
                tela.myapp.ui.label_equacao_entrada.setText('Entrada: NOK')
...
##                        return
            ...
            ### Status Coletas ###
##            if (self.ui.Chk_Auto.isChecked()) and (self.ui.Chk_Auto.isEnabled()):
##                self.ui.Hab_Corretora.setEnabled(False)
##                self.ui.C_Sucessivas.setEnabled(False)
##                time.sleep(.1)
##            else:
##                self.ui.Hab_Corretora.setEnabled(True)
##                self.ui.C_Sucessivas.setEnabled(True)
##            if (self.ui.C_Sucessivas.isChecked()) and (self.ui.C_Sucessivas.isEnabled()):
##                self.ui.N_Coletas_Manual.setEnabled(True)
##                self.ui.label_22.setEnabled(True)
##                self.ui.Chk_Auto.setEnabled(False)
##                self.ui.Hab_Corretora.setEnabled(False)
