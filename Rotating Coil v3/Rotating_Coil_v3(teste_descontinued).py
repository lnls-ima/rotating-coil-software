...
from Progress_bar import Ui_progress_bar
...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def send_setpoint(self, secondary=False):
        ...
        try:
            ...
            _slew = self.slew_rate(_slewrate)
    ...
    def progressBarDialog(self, deadline, tempo):
        try:
            self.pbar = QtWidgets.QDialog()
            self.pbar.ui = Ui_progress_bar()
            self.pbar.ui.setupUi(self.pbar)
            self.pbar.setAttribute(QtCore.Qt.WA_DeleteOnClose)
            self.pbar.show()
            self.completed = 0
            _counter = 0
            self.pbar.ui.progressBar.setValue(self.completed)
            QtWidgets.QApplication.processEvents()
            while time.monotonic() < deadline:
                self.completed = (100*_counter)/(tempo*60)
                self.pbar.ui.progressBar.setValue(self.completed)
                QtWidgets.QApplication.processEvents()
                _counter += 1
                time.sleep(0.016)
    ...
    def cycling_ps(self, secondary=False):
        ...
        try:
            ...
            if _curve_type == 3:
                ...
                _tempo = (1/_freq*_n_cycles)
                self.progressBarDialog(_deadline, _tempo)
    ...
    def move_motor_until_stops(self, address):
        ...
        _count = 50
        while ((_stop_flag is False) and (Lib.flags.stop_all is False)) and (_count > 0):
    ...
    def start_meas(self):
        ...
        try:
            ...
            for i in range(_n_collections):
                ...
                if self.ui.chb_automatic_ps.isChecked():
                    #*Change suggested by James*: Slew rate option
                    _sr = self.ui.dsb_current_slewrate.value()
                    self.slew_rate(_sr)
