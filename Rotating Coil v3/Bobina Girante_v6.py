...
##            print(reshape)
    ...
    def Coleta_tensao(self):                    #ACRESCENTADO#
        ...
        valor_tensao = value.split(',')
        if len(valor_tensao) >= 1:
            tensao = float(valor_tensao[-1])
...
########################## Corretoras ###############################
    ...
    def COLETA_INTEGRADOR(self):
        ...
        if self.ui.tempLine.text() == '':                           #ACRESCENTADO#
            ...
            return False
        if self.ui.check_tensao.isChecked():                           #ACRESCENTADO#
...
##            print(arquivo)
            ...
            if lib.Janela.ui.check_tensao.isChecked() and lib.Janela.ui.check_aux.isChecked():
                ...
                f.write('magnet_resistance_std(ohm)     \t' + str('{0:+0.6e}'.format(resis_deviation)) + '\n')
                f.write('trim_coil_current_avg(A)   \t' + str('{0:+0.6e}'.format(numpy.mean(lib.Leitura_tensao_e_corrente.corrente_trim))) + '\n')
                f.write('trim_coil_current_std(A)   \t' + str('{0:+0.6e}'.format(numpy.std(lib.Leitura_tensao_e_corrente.corrente_trim))) + '\n')

            elif lib.Janela.ui.check_tensao.isChecked():
            ...
            if imabobina == 1 and lib.Janela.ui.check_aux.isChecked() and TipoIma == 3:             #PARA SEXTUPOLO DO ANEL S15#
...
##            lib.LeituraCorrente_Secundaria = leitura_corrente_sec_mult(lib.voltas_offset,lib.velocidade)              #Para leitura da corrente na proópria fonte iLoad1() - ACRESCENTADO#
        ...
        if lib.controle_fonte == 1 and lib.Janela.ui.check_aux.isChecked() and lib.Janela.ui.check_TrimCoil.isChecked():
            if not lib.Janela.ui.check_tensao.isChecked():
                lib.LeituraCorrente_Secundaria = leitura_corrente_sec_Multicanal(lib.voltas_offset,lib.velocidade)      #Para leitura da corrente com o multicanal - ACRESCENTADO#
            else:
...
class multicanal_tensao_e_corrente(threading.Thread):
    ...
    def run(self):
        ...
        tensao = []
        for i in range(self.nVoltas):
            leitura = lib.GPIB.Coleta_Multicanal()  #Ler Multicanal com os dois canais habilitados
...
#################### Leitura Tensão (Multicanal) ################### ACRESCENTADO
class leitura_tensao_multicanal(threading.Thread):
    def __init__(self, nVoltas, velocidade):
        threading.Thread.__init__(self)
        self.nVoltas = int(nVoltas)
        self.velocidade = float(velocidade)
    ...
    def run(self):
        saida = []
        for i in range(self.nVoltas):
            saida.append(lib.Janela.Coleta_tensao())
            time.sleep((1/self.velocidade)-0.01)
        self.saida = saida
